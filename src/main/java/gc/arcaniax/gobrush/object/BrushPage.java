...
public class BrushPage
{
  ...
  public BrushPage(List<Brush> brushes, int pageNumber, int pageCount)
  {
    ...
    for (int i = 0; i < brushes.size(); i++) {
      this.INVENTORY.setItem(i, createItem(XMaterial.MAP.parseMaterial(), (short)0, "&e" + ((Brush)brushes.get(i)).getName(), getImageLore(getBrush(((Brush)brushes.get(i)).getName()))));
    }
  }
